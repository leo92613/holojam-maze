...
namespace SpriteMaker{
	...
	public class TransformUniversalSetTime : MonoBehaviour {
		...
		void Update () {
			...
			if (doTranslate) {
				...
				translate += Vector3.Scale(translateSpeed,direction)*currentTime * globalTimeScale;
			...
			if (doTranslateOscillate) {
				translateOscillateCounter += translateOscillateSpeed * currentTime * globalTimeScale;
			...
			if (doTranslateNoise) {
				translateNoiseCounter += translateNoiseSpeed * currentTime * globalTimeScale;
			...
			if(doRotate){
				rotation += rotate*currentTime * globalTimeScale;
			}

			if (doRotateOscillate) {
				rotateOscillateCounter += rotateOscillateSpeed * currentTime * globalTimeScale;
			...
			if (doRotateNoise) {
				rotateNoiseCounter += rotateNoiseSpeed * currentTime * globalTimeScale;
			...
			if(doScale){
				...
				scalar += Vector3.Scale(scale,scaleDirection)*currentTime * globalTimeScale;
			...
			if (doScaleOscillate) {
				scaleOscillateCounter += scaleOscillateSpeed * currentTime * globalTimeScale;
			...
			if (doScaleNoise) {
				scaleNoiseCounter += scaleNoiseSpeed * currentTime * globalTimeScale;

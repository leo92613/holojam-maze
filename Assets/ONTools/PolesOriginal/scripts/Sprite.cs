	...
	public void setAnim(string name)
	{
		...
		for (int i = 0; i < animations.Count; ++i)
		{
			if (((UVAnimation)animations[i]).name == name){
				curAnim = (UVAnimation)animations[i];
//				timeBetweenAnimFrames = .1f ;
	...
	public void PlayAnim(UVAnimation anim, float t)
	{
		// First stop any currently playing animation:
		m_manager.StopAnimation(this);
	...
	public void PlayAnim(string name,float t)
	{
		for (int i = 0; i < animations.Count; ++i)
		{
			if (((UVAnimation)animations[i]).name == name)
				PlayAnim((UVAnimation)animations[i],t);
		}
	}
